...
<main id="top">
    @php
        ...
        $richClasses = 'leading-relaxed text-slate-200 [&_p]:mt-3 [&_ul]:mt-3 [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:mt-3 [&_ol]:list-decimal [&_ol]:pl-5 [&_a]:text-white [&_a]:underline';
    @endphp
        ...
        <div class="mx-auto grid max-w-7xl items-center gap-10 px-4 py-14 sm:px-6 lg:grid-cols-12 lg:px-8 lg:py-20">
            <div class="lg:col-span-7">
                ...
                <h1 class="reveal mt-6 text-4xl font-semibold tracking-tight text-white sm:text-5xl lg:text-6xl">
                    <span class="text-transparent bg-clip-text bg-gradient-to-r from-indigo-300 via-sky-300 to-emerald-300 animate-gradient">{{ optional($hero)->title ?: __('frontend.about_title') }}</span>
                </h1>
                ...
                @else
                    <p class="reveal mt-5 max-w-2xl text-base leading-relaxed text-slate-200 sm:text-lg">{{ $heroContent }}</p>
                @endif
                ...
                <div class="reveal mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
                    ...
                    <a href="#story" class="inline-flex items-center justify-center rounded-2xl bg-white/10 px-6 py-3 text-sm font-semibold text-white ring-1 ring-white/10 transition hover:bg-white/15">
                ...
                <div class="reveal relative overflow-hidden rounded-3xl bg-white/5 p-6 ring-1 ring-white/10">
                    ...
                    <div class="relative">
                        <div class="flex items-center justify-between">
                            <div>
                                <div class="text-xs text-slate-300">{{ __('frontend.about_page_stats_title') }}</div>
                                <div class="mt-1 text-lg font-semibold text-white">{{ optional($statsTitle)->title ?: __('frontend.about_page_stats_title') }}</div>
                            </div>
                        ...
                        <div class="mt-6 grid gap-4">
                            <div class="rounded-2xl bg-white/10 p-4 ring-1 ring-white/10">
                                <div class="flex items-start gap-3">
                                    ...
                                    <div>
                                        <div class="text-sm font-semibold text-white">{{ optional($mission)->title ?: __('frontend.about_page_mission_title') }}</div>
                                        @php
                                        ...
                                        @else
                                            <div class="mt-1 text-sm text-slate-200">{{ $missionContent }}</div>
                                        @endif
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="rounded-2xl bg-white/10 p-4 ring-1 ring-white/10">
                                <div class="flex items-start gap-3">
                                    ...
                                    <div>
                                        <div class="text-sm font-semibold text-white">{{ optional($vision)->title ?: __('frontend.about_page_vision_title') }}</div>
                                        @php
                                        ...
                                        @else
                                            <div class="mt-1 text-sm text-slate-200">{{ $visionContent }}</div>
                                        @endif
                                    </div>
                                </div>
                            </div>
    ...
    <!-- Story -->
    <section id="story" class="border-t border-white/10">
            ...
            <div class="reveal grid gap-10 lg:grid-cols-12 lg:items-start">
                <div class="lg:col-span-5">
                    <div class="rounded-3xl bg-white/5 p-6 ring-1 ring-white/10">
                        <div class="text-xs font-semibold text-slate-300">{{ __('frontend.about') }}</div>
                        <h2 class="mt-2 text-2xl font-semibold text-white sm:text-3xl">{{ optional($intro)->title ?: __('frontend.about_page_intro_title') }}</h2>
                ...
                <div class="lg:col-span-7">
                    ...
                    @endphp
                    @if($introIsHtml)
                        <div class="rounded-3xl bg-white/5 p-8 ring-1 ring-white/10 {{ $richClasses }}">{!! $introContent !!}</div>
                    @else
                        <div class="rounded-3xl bg-white/5 p-8 ring-1 ring-white/10">
                            <p class="text-base leading-relaxed text-slate-200 sm:text-lg">{{ $introContent }}</p>
                        </div>
                    @endif
                </div>
            </div>
    ...
    <section class="border-t border-white/10">
        <div class="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
            <div class="reveal flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <h2 class="text-2xl font-semibold text-white sm:text-3xl">{{ __('frontend.about_page_values_title') }}</h2>
                    <p class="mt-2 max-w-3xl text-slate-200">{{ __('frontend.about_page_values_subtitle') }}</p>
                </div>
            </div>

            @php
                $valueAccent = [
                    ['bg' => 'bg-indigo-500/20', 'ring' => 'ring-indigo-400/20', 'text' => 'text-indigo-100'],
                    ['bg' => 'bg-sky-500/20', 'ring' => 'ring-sky-400/20', 'text' => 'text-sky-100'],
                    ['bg' => 'bg-emerald-500/20', 'ring' => 'ring-emerald-400/20', 'text' => 'text-emerald-100'],
                    ['bg' => 'bg-violet-500/20', 'ring' => 'ring-violet-400/20', 'text' => 'text-violet-100'],
                ];
            @endphp
            ...
            <div class="reveal mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                @foreach($valuesKeys as $idx => $key)
                    ...
                    <div class="group rounded-3xl bg-white/5 p-6 ring-1 ring-white/10 transition hover:bg-white/[0.07]">
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                <div class="text-sm font-semibold text-white">{{ optional($valueSection)->title ?: $fallbackTitle }}</div>
                                @if($descIsHtml)
                                    <div class="mt-2 text-sm {{ $richClasses }}">{!! $desc !!}</div>
                                @else
                                    <p class="mt-2 text-sm text-slate-200">{{ $desc }}</p>
                                @endif
                            </div>
                        </div>
                    </div>
    ...
    <section class="border-t border-white/10">
        <div class="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
            <div class="reveal overflow-hidden rounded-3xl bg-gradient-to-r from-indigo-500/15 via-sky-500/10 to-emerald-500/15 p-8 ring-1 ring-white/10">
                <div class="grid gap-6 lg:grid-cols-12 lg:items-center">
                    <div class="lg:col-span-8">
                        <h2 class="text-2xl font-semibold text-white">{{ optional($cta)->title ?: __('frontend.about_page_cta_title') }}</h2>
                        @php
                        ...
                        @else
                            <p class="mt-3 text-slate-200">{{ $ctaContent }}</p>
                        @endif
                    </div>
                    <div class="lg:col-span-4 lg:text-right">
                        ...
                        <div class="mt-3 text-xs text-slate-300">{{ optional($statsTitle)->content ?: __('frontend.about_page_stats_subtitle') }}</div>
                    </div>
                </div>
            </div>
        </div>
