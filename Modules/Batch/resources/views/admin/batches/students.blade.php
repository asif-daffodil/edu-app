<x-app-layout>
    @php($availableStudentSearchIndex = ($availableStudents ?? collect())
        ->map(fn ($s) => ['id' => $s->id, 'text' => strtolower($s->name . ' ' . $s->email)])
        ->values())
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"
             x-data="{
                ...
                query: '',
                minQuery: 1,
                students: @js($availableStudentSearchIndex),
                hasMatches() {
                    const q = this.query.trim().toLowerCase();
                    if (q.length < this.minQuery) return false;
                    return this.students.some((s) => (s.text || '').includes(q));
                }
             }"
             @keydown.escape.window="addOpen = false">
            <div>
                <h2 class="text-xl font-semibold text-slate-900 leading-tight">Batch Students</h2>
                    ...
                    <div class="relative mx-auto mt-16 w-full max-w-2xl px-4">
                        <div class="rounded-2xl bg-white shadow-xl ring-1 ring-slate-200">
                            <div class="flex items-start justify-between gap-3 border-b border-slate-200 p-5">
                                <div>
                                    <h3 class="text-lg font-semibold text-slate-900">Add student</h3>
                                    <p class="mt-1 text-sm text-slate-500">Search and add a student to this batch.</p>
                                </div>
                            ...
                            <div class="p-5">
                                ...
                                <div class="mt-4 rounded-xl ring-1 ring-slate-200">
                                    @php($list = $availableStudents ?? collect())
                                    @if($list->count() === 0)
                                        <div class="p-4 text-sm text-slate-600">No available students to add.</div>
                                    @else
                                        ...
                                        <div class="max-h-[380px] overflow-auto divide-y divide-slate-200"
                                            ...
                                            @foreach($list as $student)
                                                <div class="p-4 flex items-center justify-between gap-3"
                                                     x-show="query.trim().length >= minQuery && ('{{ strtolower($student->name . ' ' . $student->email) }}').includes(query.trim().toLowerCase())">
                                                    <div class="min-w-0">
    ...
    <div class="space-y-6">
        @if(isset($pendingStudents) && $pendingStudents->count())
            <div class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
                <div class="flex items-center justify-between">
                    <h3 class="text-sm font-semibold text-slate-900">Pending approvals</h3>
                    <span class="rounded-full bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-800 ring-1 ring-amber-100">{{ $pendingStudents->count() }}</span>
                </div>
                        ...
                        <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                            ...
                            <div class="mt-4 flex items-center gap-2">
                                <form method="POST" action="{{ route('dashboard.batches.students.approve', [$batch, $student]) }}">
                                    @csrf
                                    @method('PUT')
                                    <button type="submit" class="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-semibold text-white hover:bg-emerald-500">Approve</button>
                                </form>
                                <form method="POST" action="{{ route('dashboard.batches.students.reject', [$batch, $student]) }}" onsubmit="return confirm('Reject this student?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-semibold text-rose-800 hover:bg-rose-100">Reject</button>
                                </form>
                            </div>
                        </div>
                    @endforeach
        ...
        <div class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            <div class="flex items-center justify-between gap-3">
                <div>
                    <h3 class="text-sm font-semibold text-slate-900">Admitted students</h3>
                    <p class="mt-1 text-sm text-slate-500">Only approved students are shown here.</p>
                </div>
                <span class="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700 ring-1 ring-slate-200">{{ isset($approvedStudents) ? $approvedStudents->count() : 0 }}</span>
            </div>
                ...
                <div class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach($approvedStudents as $student)
                        <div class="rounded-xl border border-slate-200 bg-white p-4">
                            ...
                            <div class="mt-4 flex items-center justify-end">
                                <form method="POST" action="{{ route('dashboard.batches.students.remove', [$batch, $student]) }}" onsubmit="return confirm('Remove this student from the batch?')">
                                    @csrf
                                    @method('DELETE')
