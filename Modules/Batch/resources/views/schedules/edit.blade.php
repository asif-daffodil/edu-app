    ...
    <div class="max-w-3xl">
        <form method="POST" action="{{ route('dashboard.batches.schedules.update', [$batch, $classSchedule]) }}" class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-slate-200 space-y-5">
            ...
            <div>
                ...
                <input name="live_class_link" value="{{ old('live_class_link', $classSchedule->live_class_link) }}" class="mt-1 w-full rounded-lg border-slate-300" />
            ...
            <div class="flex items-center justify-end gap-2">
                <a href="{{ route('dashboard.batches.schedules.show', [$batch, $classSchedule]) }}" class="rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">Cancel</a>
