        ...
        <div class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            <form method="POST" action="{{ route('dashboard.mentors.update', $mentor) }}" class="space-y-5">
                ...
                <div>
                    ...
                    <textarea name="bio" rows="5" class="mt-1 w-full rounded-lg border-slate-300 focus:border-indigo-500 focus:ring-indigo-500" placeholder="Short description...">{{ old('bio', $mentor->bio) }}</textarea>
                    @error('bio')
                        <p class="mt-1 text-sm text-rose-600">{{ $message }}</p>
                    @enderror
                </div>
